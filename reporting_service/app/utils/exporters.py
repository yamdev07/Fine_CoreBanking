...
def export_trial_balance_excel(report: dict) -> bytes:
    ...
    ws["A3"] = (
        f"Période : {report['header']['period_start']} "
        f"au {report['header']['period_end']}"
    )
    ...
    headers = [
        ...
        "Solde ouverture D", "Solde ouverture C",
        "Mouvement D", "Mouvement C",
        "Cumul D", "Cumul C",
        "Solde clôture D", "Solde clôture C",
    ]
    ...
    for line in report["lines"]:
        ...
        if cls != current_class:
            current_class = cls
            cls_labels = {
                "1": "CLASSE 1 — Capitaux", "2": "CLASSE 2 — Actifs immobilisés",
                "3": "CLASSE 3 — Opérations interbancaires",
                "4": "CLASSE 4 — Opérations clientèle",
                "5": "CLASSE 5 — Trésorerie",
                "6": "CLASSE 6 — Charges", "7": "CLASSE 7 — Produits",
            }
        ...
        values = [
            ...
            float(line["opening_debit"]),  float(line["opening_credit"]),
            float(line["period_debit"]),   float(line["period_credit"]),
            float(line["cumulative_debit"]),float(line["cumulative_credit"]),
    ...
    totals = [
        "TOTAUX", "",
        float(report["total_opening_debit"]),  float(report["total_opening_credit"]),
        float(report["total_period_debit"]),   float(report["total_period_credit"]),
        float(report["total_closing_debit"]),  float(report["total_closing_credit"]),
        float(report["total_closing_debit"]),  float(report["total_closing_credit"]),
    ]
...
def export_journal_centralizer_excel(report: dict) -> bytes:
    ...
    headers = ["Code journal", "Libellé", "Nb écritures", "Total Débit", "Total Crédit", "Équilibré"]
    ...
    for i, line in enumerate(report["lines"], start=5):
        ...
        values = [
            line["journal_code"], line["journal_name"],
            line["nb_ecritures"],
    ...
    totals = [
        "TOTAL", "", report["total_ecritures"],
        float(report["grand_total_debit"]), float(report["grand_total_credit"]),
...
def export_bceao_pdf(report: dict) -> bytes:
    ...
    title_style = ParagraphStyle(
        ...
        textColor=DARK_BLUE, fontSize=16, spaceAfter=6,
    )
    ...
    story.append(Paragraph(
        f"Agrément N° {report['institution_agree']} | "
        f"Date d'arrêté : {report['date_arrete']}",
        subtitle_style,
    ))
    ...
    # Table des ratios
    ratio_keys = [
        "ratio_solvabilite", "ratio_liquidite", "ratio_transformation",
    ...
    tbl = Table(table_data, colWidths=[
        1.5*cm, 9*cm, 2.5*cm, 2.5*cm, 3*cm
    ])
    ...
    if report.get("observations"):
        ...
        story.append(Paragraph(
            f"<b>Observations :</b> {report['observations']}", body_style
        ))

    story.append(Spacer(1, 0.5*cm))
    story.append(Paragraph(
        f"Rapport généré le {report['header']['generated_at']} "
        f"par {settings.EXPORT_AUTHOR}",
        subtitle_style,
    ))
...
def export_generic_pdf(report: dict, title: str) -> bytes:
    ...
    title_s = ParagraphStyle("t", parent=styles["Heading1"],
        textColor=DARK_BLUE, fontSize=14, spaceAfter=4)
