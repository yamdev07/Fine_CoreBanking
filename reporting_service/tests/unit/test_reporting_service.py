...
async def test_dashboard_taux_impayes():
    ...
    with (
        patch.object(svc.repo, "get_fiscal_year_for_date", new=AsyncMock(return_value=mock_fy)),
        patch.object(svc.repo, "get_credit_portfolio", new=AsyncMock(return_value=mock_credits)),
        patch.object(svc.repo, "get_deposits_by_type", new=AsyncMock(return_value=mock_depots)),
        patch.object(svc.repo, "get_cash_balance", new=AsyncMock(return_value=Decimal("200000"))),
        patch.object(svc.repo, "get_net_income", new=AsyncMock(return_value=Decimal("50000"))),
        patch.object(svc.repo, "get_equity", new=AsyncMock(return_value=Decimal("300000"))),
        patch.object(svc.repo, "get_total_assets", new=AsyncMock(return_value=Decimal("1500000"))),
        patch.object(svc.repo, "get_provisions", new=AsyncMock(return_value=Decimal("50000"))),
        patch.object(
            svc.repo, "get_interest_charges", new=AsyncMock(return_value=Decimal("10000"))
        ),
        patch.object(svc.repo, "get_charges_produits", new=AsyncMock(return_value=[])),
    ):
...
async def test_credit_portfolio_deficit_provisionnement():
    ...
    with (
        patch.object(svc.repo, "get_credit_portfolio", new=AsyncMock(return_value=mock_credits)),
        patch.object(svc.repo, "get_provisions", new=AsyncMock(return_value=Decimal("100000"))),
    ):
...
async def test_bceao_solvabilite_conforme():
    ...
    with (
        patch.object(svc.repo, "get_fiscal_year_for_date", new=AsyncMock(return_value=mock_fy)),
        patch.object(svc.repo, "get_equity", new=AsyncMock(return_value=Decimal("100000"))),
        patch.object(svc.repo, "get_total_assets", new=AsyncMock(return_value=Decimal("1000000"))),
        patch.object(svc.repo, "get_cash_balance", new=AsyncMock(return_value=Decimal("200000"))),
        patch.object(
            svc.repo,
            "get_credit_portfolio",
            new=AsyncMock(return_value=[{"account_code": "251100", "encours": "600000"}]),
        ),
        patch.object(
            svc.repo,
...
async def test_bceao_solvabilite_non_conforme():
    ...
    with (
        patch.object(svc.repo, "get_fiscal_year_for_date", new=AsyncMock(return_value=mock_fy)),
        patch.object(svc.repo, "get_equity", new=AsyncMock(return_value=Decimal("50000"))),
        patch.object(svc.repo, "get_total_assets", new=AsyncMock(return_value=Decimal("2000000"))),
        patch.object(svc.repo, "get_cash_balance", new=AsyncMock(return_value=Decimal("100000"))),
        patch.object(
            svc.repo,
            "get_credit_portfolio",
            new=AsyncMock(return_value=[{"account_code": "251100", "encours": "1500000"}]),
        ),
        patch.object(
            svc.repo,
            "get_deposits_by_type",
            new=AsyncMock(return_value=[{"code": "371100", "encours": "800000"}]),
        ),
    ):
        report = await svc.bceao_report(date(2024, 12, 31), "IMF-BJ-2024-001")
