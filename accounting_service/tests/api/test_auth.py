...
class TestLogin:
    ...
    async def test_login_inactive_user_returns_401(self, api_client: AsyncClient, admin_headers: dict):
        ...
        res = await api_client.post("/api/v1/auth/login", json={
            "username": "accountant", "password": "Acc1234!",
        })
        assert res.status_code == 401
...
class TestMe:
    ...
    async def test_me_with_expired_token_returns_401(self, api_client: AsyncClient, expired_token_headers: dict):
    ...
    async def test_me_with_malformed_token_returns_401(self, api_client: AsyncClient):
        res = await api_client.get("/api/v1/auth/me", headers={"Authorization": "Bearer not.a.valid.jwt"})
        assert res.status_code == 401
        assert res.json()["detail"]["error_code"] == "TOKEN_INVALID"

    async def test_me_returns_correct_role_for_accountant(self, api_client: AsyncClient, accountant_headers: dict):
