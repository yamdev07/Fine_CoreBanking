...
class TestCreateFiscalYear:
    async def test_create_fiscal_year_success(self, api_client: AsyncClient, admin_headers: dict):
        res = await api_client.post(
            "/api/v1/fiscal-years/", json=VALID_FISCAL_YEAR, headers=admin_headers
        )
    ...
    async def test_create_fiscal_year_generates_12_periods(
        ...
        periods_res = await api_client.get(
            f"/api/v1/fiscal-years/{fy_id}/periods", headers=admin_headers
        )
    ...
    async def test_create_fiscal_year_invalid_dates_returns_422(
        self, api_client: AsyncClient, admin_headers: dict
    ):
        res = await api_client.post(
            "/api/v1/fiscal-years/",
    ...
    async def test_create_fiscal_year_auditor_forbidden(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
        res = await api_client.post(
            "/api/v1/fiscal-years/", json=VALID_FISCAL_YEAR, headers=auditor_headers
        )
...
class TestGetFiscalYear:
    async def test_get_by_id(self, api_client: AsyncClient, auditor_headers: dict):
        res = await api_client.get(
            f"/api/v1/fiscal-years/{FISCAL_YEAR_ID}", headers=auditor_headers
        )
        assert res.status_code == 200
    ...
    async def test_get_nonexistent_returns_404(
        ...
        res = await api_client.get(
            "/api/v1/fiscal-years/00000000-0000-0000-0000-deadbeef0001", headers=auditor_headers
        )
        assert res.status_code == 404

    async def test_list_periods_of_seeded_fy(self, api_client: AsyncClient, auditor_headers: dict):
        res = await api_client.get(
            f"/api/v1/fiscal-years/{FISCAL_YEAR_ID}/periods", headers=auditor_headers
        )
        assert res.status_code == 200
        periods = res.json()
    ...
    async def test_periods_of_nonexistent_fy_returns_404(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
        res = await api_client.get(
            "/api/v1/fiscal-years/00000000-0000-0000-0000-deadbeef0001/periods",
...
class TestCloseFiscalYear:
    async def test_close_fiscal_year_success(self, api_client: AsyncClient, admin_headers: dict):
        ...
        close_res = await api_client.post(
            f"/api/v1/fiscal-years/{fy_id}/close", headers=admin_headers
        )
    ...
    async def test_close_fiscal_year_auditor_forbidden(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
        res = await api_client.post(
            f"/api/v1/fiscal-years/{FISCAL_YEAR_ID}/close", headers=auditor_headers
        )
        assert res.status_code == 403
