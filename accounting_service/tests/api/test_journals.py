...
def balanced_entry(debit_account: str = CASH_ACCOUNT_ID, credit_account: str = CREDIT_ACCOUNT_ID, amount: str = "500000") -> dict:
    return {
...
class TestCreateEntry:
    ...
    async def test_create_entry_success(self, api_client: AsyncClient, accountant_headers: dict):
        res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
    ...
    async def test_create_imbalanced_entry_returns_422(self, api_client: AsyncClient, accountant_headers: dict):
        data = {
            "journal_id": CAISSE_JOURNAL_ID,
            "entry_date": "2024-01-15",
        ...
        res = await api_client.post("/api/v1/journal-entries/", json=data, headers=accountant_headers)
        assert res.status_code == 422

    async def test_create_entry_outside_period_returns_404(self, api_client: AsyncClient, accountant_headers: dict):
        data = balanced_entry()
        data["entry_date"] = "2023-06-01"  # No open period for this date
        res = await api_client.post("/api/v1/journal-entries/", json=data, headers=accountant_headers)
        assert res.status_code == 404

    async def test_create_entry_auditor_forbidden(self, api_client: AsyncClient, auditor_headers: dict):
        res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=auditor_headers)
    ...
    async def test_create_entry_single_line_rejected(self, api_client: AsyncClient, accountant_headers: dict):
        data = {
            "journal_id": CAISSE_JOURNAL_ID,
            "entry_date": "2024-01-15",
...
class TestListEntries:
    ...
    async def test_list_entries_by_period(self, api_client: AsyncClient, auditor_headers: dict, accountant_headers: dict):
        # Create an entry first
        await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)

        res = await api_client.get(f"/api/v1/journal-entries/?period_id={PERIOD_ID}", headers=auditor_headers)
        assert res.status_code == 200
        body = res.json()
        assert body["total"] >= 1
        assert isinstance(body["items"], list)

    async def test_list_entries_invalid_status_returns_422(self, api_client: AsyncClient, auditor_headers: dict):
    ...
    async def test_list_entries_filter_by_status(self, api_client: AsyncClient, auditor_headers: dict, accountant_headers: dict):
        await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
...
class TestGetEntry:
    ...
    async def test_get_entry_by_id(self, api_client: AsyncClient, accountant_headers: dict, auditor_headers: dict):
    ...
    async def test_get_nonexistent_entry_returns_404(self, api_client: AsyncClient, auditor_headers: dict):
        res = await api_client.get("/api/v1/journal-entries/00000000-0000-0000-0000-deadbeef9999", headers=auditor_headers)
        assert res.status_code == 404


class TestPostEntry:

    async def test_post_entry_changes_status_to_posted(self, api_client: AsyncClient, accountant_headers: dict):
        create_res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
        entry_id = create_res.json()["id"]
    ...
    async def test_post_entry_twice_returns_422(self, api_client: AsyncClient, accountant_headers: dict):
        create_res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
        entry_id = create_res.json()["id"]
        await api_client.post(f"/api/v1/journal-entries/{entry_id}/post", headers=accountant_headers)

        res = await api_client.post(f"/api/v1/journal-entries/{entry_id}/post", headers=accountant_headers)
        assert res.status_code == 422

    async def test_post_entry_auditor_forbidden(self, api_client: AsyncClient, accountant_headers: dict, auditor_headers: dict):
        create_res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
        entry_id = create_res.json()["id"]

        res = await api_client.post(f"/api/v1/journal-entries/{entry_id}/post", headers=auditor_headers)
        assert res.status_code == 403
...
class TestReverseEntry:
    ...
    async def _create_and_post(self, api_client: AsyncClient, headers: dict) -> str:
        create_res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=headers)
        entry_id = create_res.json()["id"]
        await api_client.post(f"/api/v1/journal-entries/{entry_id}/post", headers=headers)
        return entry_id

    async def test_reverse_posted_entry_success(self, api_client: AsyncClient, accountant_headers: dict):
    ...
    async def test_reverse_draft_entry_returns_422(self, api_client: AsyncClient, accountant_headers: dict):
        create_res = await api_client.post("/api/v1/journal-entries/", json=balanced_entry(), headers=accountant_headers)
    ...
    async def test_reverse_already_reversed_returns_422(self, api_client: AsyncClient, accountant_headers: dict):
        entry_id = await self._create_and_post(api_client, accountant_headers)
    ...
    async def test_reverse_marks_original_as_reversed(self, api_client: AsyncClient, accountant_headers: dict, auditor_headers: dict):
        entry_id = await self._create_and_post(api_client, accountant_headers)
        await api_client.post(
            f"/api/v1/journal-entries/{entry_id}/reverse",
            params={"reversal_date": "2024-01-20"},
        ...
        original = await api_client.get(f"/api/v1/journal-entries/{entry_id}", headers=auditor_headers)
