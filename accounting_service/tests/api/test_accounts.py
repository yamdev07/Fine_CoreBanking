...
class TestCreateAccount:
    ...
    async def test_create_account_by_accountant_allowed(
        self, api_client: AsyncClient, accountant_headers: dict
    ):
        res = await api_client.post(
            "/api/v1/accounts/", json=NEW_ACCOUNT, headers=accountant_headers
        )
        assert res.status_code == 201

    async def test_create_account_by_auditor_forbidden(
    ...
    async def test_create_account_with_parent_sets_hierarchy(
        self, api_client: AsyncClient, admin_headers: dict
    ):
        parent_res = await api_client.post(
            "/api/v1/accounts/",
            json={
                "code": "41",
                "name": "Clients (classe)",
        ...
        child_res = await api_client.post(
            "/api/v1/accounts/",
            json={
                **NEW_ACCOUNT,
                "parent_id": parent_id,
    ...
    async def test_create_account_invalid_code_not_digits(
        self, api_client: AsyncClient, admin_headers: dict
    ):
        res = await api_client.post(
            "/api/v1/accounts/",
...
class TestListAccounts:
    ...
    async def test_list_accounts_search_by_code(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
...
class TestGetAccount:
    ...
    async def test_get_nonexistent_account_returns_404(
        ...
        res = await api_client.get(
            "/api/v1/accounts/00000000-0000-0000-0000-deadbeef0000", headers=auditor_headers
        )
...
class TestUpdateAccount:
    ...
    async def test_update_account_auditor_forbidden(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
        res = await api_client.patch(
            f"/api/v1/accounts/{CASH_ACCOUNT_ID}",
...
class TestDeactivateAccount:
    async def test_deactivate_requires_admin(
        ...
        res = await api_client.delete(
            f"/api/v1/accounts/{CASH_ACCOUNT_ID}", headers=accountant_headers
        )
        assert res.status_code == 403

    async def test_deactivate_account_with_no_balance(
        self, api_client: AsyncClient, admin_headers: dict
    ):
...
class TestAccountBalance:
    ...
    async def test_get_balance_missing_dates_returns_422(
        self, api_client: AsyncClient, auditor_headers: dict
    ):
        res = await api_client.get(
            f"/api/v1/accounts/{CASH_ACCOUNT_ID}/balance",
